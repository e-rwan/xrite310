...
def _set_dense_x_minor_ticks(ax, ticks: Sequence[float]):
	"""Adds intermediate vertical grid lines between major X ticks."""
	if len(ticks) < 2:
		return

	minor_ticks = [(left + right) / 2 for left, right in zip(ticks[:-1], ticks[1:])]
	if minor_ticks:
		ax.set_xticks(minor_ticks, minor=True)


def _apply_precise_grid(ax):
	"""Applies a denser grid with a finer secondary mesh."""
...
def draw_curve_graph(
	...
	ax.yaxis.set_minor_locator(MultipleLocator(step / GRID_MINOR_STEP))
	...
	if x_is_string and x_labels:
		major_ticks = list(range(len(x_labels)))
		ax.set_xticks(major_ticks)
		ax.set_xticklabels(x_labels, rotation=45, ha='right')
		_set_dense_x_minor_ticks(ax, major_ticks)
		...
		if x_tick_labels:
			...
			major_ticks = list(range(tick_start, tick_end + 1))
			labels = [x_tick_labels[i - 1] if 1 <= i <= tick_count else str(i) for i in major_ticks]
